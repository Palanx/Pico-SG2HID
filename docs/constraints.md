...
- **R-PROC-01** — Every rule in this file has exactly one binding in ADR-0005's grammar; every `test:` file carries a matching `RULE <id>` marker; every marker names a declared id; `manual:` rules carry no marker; every `planned:` phase exists and is not `done`; every id `CLAUDE.md` mentions is declared here. **Scope, recorded 2026-09-24 because the check reads less than this text says:** the check is `check( )` in `tests/test_rule_traceability.py`. It sees a rule only on a line matching `ANY_RULE_LINE`, which needs `- **R-` at the start of the line, so a rule written as `* **R-X-01**` or indented under another bullet is not read at all: neither its binding nor its missing test is reported. Measured one fixture per form through `make_repo( )` and the real `check( )`, each not reported: a `manual:` rule whose marker sits under `docs/`, `.claude/commands/`, `build/` or a `node_modules/` directory (`SKIP_PREFIXES`, `SKIP_DIRS`); a `test:` file whose only marker names a longer id, where `R-X-011` satisfies `R-X-01` because neither `MARKER` nor the substring test is bounded after the two digits; a line carrying a second binding after its first, which the lazy `RULE_LINE` reads as one binding whose value swallows the second; a `planned:` phase whose status is spelled `Done`, since the status is compared exactly; and a `CLAUDE.md` mention in range notation (`R-X-01..03`), from which `RULE_ID` extracts only the first id. The check proves that the bound file exists and contains the marker text, not that `make test` runs that file or that it tests the rule. It also errs loudly: a `test:` path that is a directory crashes it with `IsADirectoryError` instead of producing a finding. — test: `tests/test_rule_traceability.py`
