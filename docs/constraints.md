...
- **R-CLEAN-04** — No magic numbers or strings in logic. Every protocol byte, timeout and threshold is a named `constexpr` in one place per concern. The literals under `tests/vectors/` are the sole exception, and being literal is their purpose (R-PROTO-05). Enforced by `readability-magic-numbers`, which does not see a literal inside a `const` or `constexpr` initializer — the gap is measured and recorded in §Observed conventions, 2026-09-11, rather than left for a reader to assume away. **Scope, recorded 2026-09-15 because the check reads less than this text says:** the rule is written for the whole repository, and the check runs `clang-tidy` over `tidy_sources()` only — `src/core/*.cpp`, `src/core/*.h` and `tests/*.cpp` (`tests/test_style.sh:68`). Anything outside those three globs is unchecked, including every future layer under `src/`. A second narrowing on the same axis: `.clang-tidy` sets `HeaderFilterRegex: 'src/.*'`, which governs the headers an analysed file *includes*, so since `tests/*.h` is never passed as a main file, a header under `tests/` is diagnosed only when its path matches that regex. **Whether it does depends on where the repository is checked out** (measured 2026-09-16): the regex is unanchored and clang-tidy matches it against the header's absolute path. In a clone whose path contains no `src/`, a naming violation planted in `tests/vectors/digital_idle.h` passes `make lint`; the same violation in a clone under `…/src/pico-sg2hid` fails it with `FAIL: R-STYLE-02 / R-CLEAN-02 / R-CLEAN-04`. The unmodified tree is green in both checkouts, because every vector literal sits in a `constexpr` initializer, which `readability-magic-numbers` does not see (above), so the stated exception holds either way today. For `tests/vectors/` the exclusion is deliberate and is this rule's own stated exception; for any other header under `tests/` it would be silent in the first checkout and diagnosed in the second, which is why `01-ps2-codec` binds a criterion asserting there are none. — test: `tests/test_style.sh`
