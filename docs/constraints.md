...
- **R-STYLE-02** — Naming matches `.clang-tidy`: private and protected members carry the `m_` prefix, types are `CamelCase`, functions and variables are `lower_case`, compile-time constants are `kCamelCase`, macros are `UPPER_CASE`. **Scope, recorded 2026-09-23 because the check reads less than this text says:** the check runs `clang-tidy` over `tidy_sources()` only — `src/*.cpp`, `src/*.h` and `tests/*.cpp` (`tests/test_style.sh`), git pathspecs read recursively as R-CLEAN-04 records, so every layer under `src/` is reached — plus any header under `src/` or `tests/` that one of those files includes (`HeaderFilterRegex` in `.clang-tidy`). Every other file extension (`.hpp`, `.cc`, `.inl`, `.c`) and every non-C++ file is unchecked: a name in those files that breaks this rule is reported by nothing. — test: `tests/test_style.sh`
...
R-STYLE-02's scope is every `.cpp` and `.h` under `src/` plus `tests/*.cpp`. clang-tidy
