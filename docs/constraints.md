...
- **R-SAFETY-01** — The Pico must never configure `DATA` or `ACK` as a push-pull output; both are open-drain lines driven by the controller and read by us. **Scope:** the check reads `kMasterPins` only — the emulator's table is R-SAFETY-06's, and what `src/hal/` does with the table is R-SAFETY-10's. — test: `tests/test_pin_table.py`
- **R-SAFETY-02** — Every GPIO used by this project is declared exactly once in `src/core/pins.h` with its signal name, direction and drive mode, and `docs/wiring.md`'s pin table names the same GPIO for each signal. **Split 2026-09-24 (operator decision, phase `02-wiring`):** the clause forbidding pin configuration outside `src/hal/` moved to R-SAFETY-09, and the clause that `src/hal/` configures pins only by iterating the table moved to R-SAFETY-10, because ADR-0005 allows one `test:` path per rule. **Scope:** the table half reads `kMasterPins` only and requires each of the five bus signals exactly once; the wiring-doc half compares it with `docs/wiring.md` rows of the shape `| SIGNAL | GP<n> |` and sees no row written any other way. — test: `tests/test_pin_table.py`
- **R-SAFETY-03** — No entry in a pin table may use a GPIO the Pico board reserves (23, 24, 25, 29), and no two signals in one table may share a GPIO. **Scope:** the reserved list is fixed in the test (23, 24, 25, 29), not read from any board file, and only `kMasterPins` is checked. — test: `tests/test_pin_table.py`
...
- **R-SAFETY-08** — Firmware is never flashed to a Pico while the guitar is connected, and the guitar is connected only after the firmware is running and has been verified against the emulator. This ordering governs the signal lines; the phase-`02-wiring` current-draw measurement connects 3V3 and GND only, with the Pico in BOOTSEL, per ADR-0013. — manual: an ordering of physical acts; enforced by the per-phase `verify.md` procedure, which the build cannot observe.
- **R-SAFETY-09** — No file under `src/` outside `src/hal/` calls a Pico SDK function that configures a pin: `gpio_init*`, `gpio_set_dir*`, `gpio_set_function*`, `gpio_set_pulls`, `gpio_pull_up`, `gpio_pull_down`, `gpio_disable_pulls`, `gpio_set_oeover`, `pio_gpio_init`, `pio_sm_set_pindirs_with_mask*` or `pio_sm_set_consecutive_pindirs`, each matched as the name followed by optional whitespace and `(`, a trailing `*` meaning any `[a-z0-9_]` suffix. **Split out of R-SAFETY-02 and widened 2026-09-24 (operator decision, phase `02-wiring`):** R-SAFETY-02 named only `gpio_init`, `gpio_set_dir` and `gpio_pull_up`. **Scope, recorded 2026-09-25 because the check reads less than this text says:** the check is `find_safety09( )` in `tests/test_repo_shape.sh`, a grep over `src_files( )` (`.cpp .h .hpp .cc .inl`) minus paths under `src/hal/`, with `//` comments stripped. Measured one fixture per form: a call with its arguments on the next line is reported; a call whose `(` is on the line after the name is not; a call through a macro (`#define INIT_PIN gpio_init`) or a function pointer (`&gpio_init`) is not; a direct register write (`sio_hw->gpio_oe_set`, `iobank0_hw`, `padsbank0_hw`) is not; a `set pindirs` instruction in a `.pio` file is not, nor is any `.c` file, since neither extension is scanned; a call inside `/* … */` or a string literal is reported (a false positive). The names are the Pico SDK's as the rule states them, not yet checked against installed SDK headers — `03-pio-bus` does that. — test: `tests/test_repo_shape.sh`
- **R-SAFETY-10** — `src/hal/` configures pins only by iterating the `src/core/pins.h` table. **Split out of R-SAFETY-02 2026-09-24:** `src/hal/` does not exist before `03-pio-bus`. — planned: 03-pio-bus
