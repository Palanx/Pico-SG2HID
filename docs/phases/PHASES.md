...
|----|------|---------|---------------------|--------|
| 00-scaffold | Repo-shape checks and the rule↔test traceability meta-test | - | `make test` runs green from a clean clone with only clang++ and python3; every check ships a rejection case proving it fails on a bad tree, and the eleven rules marked `planned: 00-scaffold` move to `test:` | pending |
