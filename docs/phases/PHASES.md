...
|----|------|---------|---------------------|--------|
| 00-scaffold | Build entry, host test harness, and the rule↔test traceability meta-test | - | `make test` runs green from a clean clone with only clang++ and python3; the meta-test fails on a deliberately broken rule binding, and `docs/constraints.md` R-PROTO-05, R-ARCH-01..03, R-PROC-01..02 move from `planned:` to `test:` | pending |
